...
namespace WebApplication1.Controllers
{
    ...
    public class DentistController : BaseController
    {
        ...
        public ActionResult MyAppointments(string sortOrder, string searchString)
        {
            ...
            var pavm = query.Select(item => new PatientAppointmentViewModel
            {
                Id = item.Id, Description = item.Description, End = item.End, FirstName = item.FirstName, LastName = item.LastName, Start = item.Start, Title = item.Title, Email = item.Email
            }).ToList();
            //Sorting
            ViewBag.NameSortParm = string.IsNullOrEmpty(sortOrder) ? "name_desc" : "";
            ViewBag.LNameSortParm = string.IsNullOrEmpty(sortOrder) ? "LName_desc" : "LName";
        ...
        public ActionResult AcceptAppointment(string id)
        {
            ...
            db.SaveChanges();
            var patientEmail =
                (from patient in db.Patients where patient.Id == appointment.PatientId select new { patient.Email })
                    .Single().ToString();
            
            SendEmail(patientEmail, "Appointment accepted","Your appointment has been accepted");
        ...
        public ActionResult RejectAppointment(string id)
        {
            ...
            var patientEmail =
                (from patient in db.Patients where patient.Id == appointment.PatientId select new { patient.Email })
                    .Single().ToString();
            patientEmail = patientEmail.Substring(10).TrimEnd(' ','}');
            appointment.IsAccepted = true;
            db.SaveChanges();
            

            SendEmail(patientEmail, "Appointment rejected","Your appointment has been rejected");
            return RedirectToAction("MyAppointments");
        }
        ...
        public ActionResult SendEmail(string emailTo, string emailSubject, string emailBody)
        {
